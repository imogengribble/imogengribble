# Hi, I’m Imogen!

Product Designer focused on crafting intuitive, accessible, and scalable digital experiences.

I design end-to-end customer experiences across complex domains including travel, loyalty, wagering, and customer support. My work sits at the intersection of user needs, business goals, and technical constraints- with a strong focus on measurable outcomes and continuous iteration.

---

## 👩‍💻 About me

- Product Designer with experience across **Tabcorp, Qantas, and freelance client work**
- Focused on **UX strategy, interaction design, and end-to-end product thinking**
- Strong background in **research-led design, experimentation, and iterative delivery**
- Passionate about improving **real-world customer journeys and operational efficiency**
- Currently based in **Sydney**, relocating to **London**

---

## 💼 Selected work

### Tabcorp — Withdrawal Block Experience
- Led redesign of withdrawal restriction experience across web and app
- Improved clarity of resolution pathways for customers
- Achieved **97% reduction in digital feedback related to blocked withdrawals**
- Reduced a two-day customer feedback loop through UX improvements

### Qantas — Help & Support Experience
- Redefined support journey by consolidating fragmented channels into a unified experience
- Improved discoverability and reduced customer friction across support pathways

### Qantas — Frequent Flyer & Loyalty Experiences
- Designed end-to-end improvements across shopping, loyalty, and rewards ecosystems
- Led discovery and UX redesign work informed by Voice of Customer insights

### Design Systems (Qantas & Jetstar)
- Contributed to component libraries, accessibility standards, and design governance
- Supported scalable UI consistency across multiple product teams

---

## 🧠 What I focus on

- End-to-end UX and product thinking  
- Designing for complex systems and constraints  
- Turning research into actionable design decisions  
- Improving operational efficiency through UX  
- Accessible, inclusive design

---

## 📁 Portfolio

Selected product design work and case studies presented as interactive UX flows.

👉 View portfolio (Figma prototype best viewed on Desktop):  
https://www.figma.com/proto/LBiQDNuSi3LYMTLgfdxsjO/Portfolio?page-id=0%3A1&node-id=2-2372&viewport=220%2C154%2C0.18&t=jvCIw6w8G69xffEd-1&scaling=scale-down&content-scaling=fixed&starting-point-node-id=2%3A2372

---

## 📫 Contact

- LinkedIn: (www.linkedin.com/in/imogen-gribble-4b1b56226)
- Email: (imogengribble@icloud.com)

---

## ⚙️ GitHub note

This GitHub profile is used primarily as a supporting space for portfolio visibility.
